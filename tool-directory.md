...
**Purpose:** Place, move, rotate, flip, atomically batch-pose, align and duplicate PCB footprints; inspect pads; inspect and edit a placed footprint's graphics.
...
| `batch_set_component_poses` | Atomically set absolute position, rotation, and F.Cu/B.Cu side for multiple existing footprints on a closed board; validates the whole request before one revision-checked write and returns exact final poses. |
